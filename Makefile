...
3.9_include := $(shell python3.9 -m pybind11 --includes) -I/usr/include/eigen3
# Directories
src_dir := ./cpp_src
out_dir := ./cpp_bin
# Sourcecode
...
3.9_py_suff := .cpython-39-x86_64-linux-gnu.so
# Output Python module
3.7_out := $(out_dir)/TreeGenModule$(3.7_py_suff)
3.8_out := $(out_dir)/TreeGenModule$(3.8_py_suff)
3.9_out := $(out_dir)/TreeGenModule$(3.9_py_suff)

test_build:
	$(CC) $(CC_options) $(3.9_include) -o $(3.9_out) $(src)
